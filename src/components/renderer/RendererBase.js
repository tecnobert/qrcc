  ...
  for (let x = 0; x < nCount; x++) {
    for (let y = 0; y < nCount; y++) {
      ...
      if (
        ...
        if (posType === 0) {
          pointList.push(
            <rect width={1} height={1} key={id++} fill={posColor} x={x} y={y} />
          );
        } else if (posType === 1) {
          pointList.push(
            <circle
          ...
          pointList.push(
            <path
              ...
              d={sq25}
              stroke={posColor}
              strokeWidth={(100 / 6) * (1 - (1 - size) * 0.75)}
              fill="none"
              transform={
                "translate(" +
                String(x - 2.5) +
  ...
  return [
    {
      ...
      key: "Estilo do ponto de informação",
      default: 0,
      choices: ["Retângulo", "Volta", "Aleatória"],
    },
    {
      type: ParamTypes.TEXT_EDITOR,
      key: "Zoom do ponto de informações",
      default: 100,
    },
    {
      type: ParamTypes.TEXT_EDITOR,
      key: "Opacidade do ponto de informação",
      default: 100,
    },
    {
      type: ParamTypes.SELECTOR,
      key: "Estilo âncora",
      default: 0,
      choices: ["Retângulo", "Volta", "Planeta", "Retângulo arredondado"],
    },
    {
      type: ParamTypes.COLOR_EDITOR,
      key: "Cor do ponto de informação",
      default: "#000000",
    },
    {
      type: ParamTypes.COLOR_EDITOR,
      key: "Cor do ponto de ancoragem",
      default: "#000000",
    },
  ];
}
...
RendererRect.detail = <div>O código QR mais básico, também existem maneiras interessantes de jogar</div>;

RendererRound.detail = <div>O estilo que a grande fábrica gosta é muito adequado para colocar no meio Logo</div>;

RendererRandRound.detail = (
  <div>
    Pontos aleatórios, caos e ordem. Derivado de{" "}
  </div>
);
